...
class VirtualAgent:
    ...
    def train(self, agentSeq):
        ...
        while True:
            ...
            if len(self.agentIndex) > 0 and len(self.replay_memory_store) > 0:
                ...
                # if replay_memory_store0[store0][2] >= 50  \
                #     or replay_memory_store0[store0][5] >= 50 \
                #     or reward_counter >= 500:
                if len(self.EE1) >= 2000 :
                    print("------------训练停止！！！！！！-----------")
                    print("reward_counter: ", reward_counter)
                    # print("replay_memory_store  \n     当前状态      |动作索引|奖励|   下个状态 ： ")
                    ...
                    print("cumulativeList: ", np.mat(self.EE1))
    ...
    def plotEECompare(self):
        print("EE1", self.EE1)
        print("EE1", self.EE2)
        plt.figure(figsize=(14, 6), dpi=80)
        plt.plot(np.arange(len(self.EE2)), self.EE1, label='rp', color='green')  # 设置折线颜色和标签
        plt.plot(np.arange(len(self.EE2)), self.EE2, label='rmiup', color='blue')
        plt.ylabel('EE')
        plt.xlabel('count')
        plt.show()
        self.EE1.clear()
